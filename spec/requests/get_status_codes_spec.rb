...
describe 'GetStatusCodes action' do
  ...
  it 'the number of status codes should be the same as the number of states on Spree::Order' do
    xml.xpath('/ShipWorks/StatusCodes/StatusCode').length.should == SpreeShipworks.order_class::VALID_SHIPMENT_STATES.length
  end

  SpreeShipworks.order_class::VALID_SHIPMENT_STATES.each do |state|
